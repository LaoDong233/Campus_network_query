...
class Server(threading.Thread):
    ...
    # 获取验证码
    def send_verification_code(self, ver_code):
    ...
    def check_user(self):
        ...
        # 检查出不一致后替换密码
        if user_cursor.execute("select * from user_info where username=%s", self.username) == 1:
            if user_cursor.execute("select * from user_info where username=%s and password=%s",
                                   (self.username, self.password)) == 0:
                user_cursor.execute(f"update user_info set "
                                    f"password = '{self.password}' "
                                    f"where username = '{self.username}'")
        # 如果数据库中没有这个用户，那么将这个用户存入数据库中
        else:
            user_cursor.execute(f"insert into user_info(username, password) VALUE ('{self.username}', '{self.password}')")
        security_cursor.execute("select ban from user_security where username = %s", self.username)
        # 判断该用户是否被封禁，如果被封禁返回FALSE
        if security_cursor.fetchone() == 1:
            conn.commit()
            return False
        else:
            user_cursor.execute("update user_info set usage_time = usage_time + 1 where username = %s", self.username)
            conn.commit()
            return True

    # 简单的时间判断模块，传入一个旧的时间戳，传出是否和当前时间相差24小时
    @staticmethod
    def time_differ_day(org_time):
    ...
    def ver_network(self):
        # 初始化记忆的地址和数量
        memory_addr = str()
        memory_addr_number = 0
        security_cursor.execute(f'select address, address_number from user_security where username = {self.username}')
        # 判断该用户是否存在
        if security_cursor.execute(
                f"select address, address_number from user_security where username = '{self.username}'"
        ) != 0:
            # 将数据库中存储的用户登录IP解包
        ...
        if not memory_addr_number == 0:
            ...
            if self.address not in memory_addr:
                ...
                for i in range(len(old_memory)):
                    ...
                    memory_addr = memory_addr + "$%^" + address
            else:
                return True
        else:
            memory_addr = address
        ...
        security_cursor.execute(
            f'update user_security set '
            f'address = {memory_addr}, '
            f'address_number = {memory_addr_number} '
            f'where username = {self.username}'
        )
        # 提交
        conn.commit()
    ...
    # 验证码模块
    def verification_code(self, push_id):
        """
        从外部获取一个PushDeer的key并传入，在内部进行存储并且进行发包
        :param push_id: PushDeer的秘钥
        :return: TRUE成功，FALSE失败
        """
        self.push_key = push_id
        if security_cursor.execute(
                f"select PushDeer_id, times from user_security where username = '{self.username}'"
        ) == 1:
            usr_security = security_cursor.fetchone()
            if push_id == usr_security[0]:
                ...
                security_cursor.execute(
                    f'update user_security set '
                    f'changes_times = changes_times + 1,'
                    f"PushDeer_id = '{push_id}' "
                    f"where username = '{self.username}'"
                )
        else:
            security_cursor.execute(
                f'insert into user_security('
                f'username,'
                f'PushDeer_id)'
                f'VALUE'
                f"('{self.username}',"
        ...
        if not recode == ver:
            ...
            self.sock.send(base64.b64encode("100".encode('utf-8')))
            login = True
            security_cursor.execute(f"update user_security set "
    ...
    def check_verification_banned(self):
        security_cursor.execute(f"select changes_times from user_security where username = '{self.username}'")
        if int(security_cursor.fetchone()[0]) >= 3:
            security_cursor.execute(f"update user_security set ban = 1 where username = '{self.username}'")
            conn.commit()
    ...
    def run(self):
        ...
        try:
            ...
            self.verification_code(push_id)
            user = None
